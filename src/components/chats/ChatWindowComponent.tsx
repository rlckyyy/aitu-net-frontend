...
export const ChatWindowComponent = ({ chatRoom, chatMessages, handleSendMessage, inputMessage, setInputMessage }: ChatWindowComponentProps) => {
			...
			<div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center">
				<div className="relative mr-3">
					<div className="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
						<span className="text-sm font-semibold">{chatRoom.title??'default title'.charAt(0).toUpperCase()}</span>
					</div>
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-3">
				{chatMessages ? (
					chatMessages.map(chatMessage => (
						<div key={chatMessage.id} className={`flex ${chatMessage.senderId === user?.id ? "justify-end" : "justify-start"}`}>
							<div
					...
					<div className="flex items-center justify-center h-full">
						<div className="text-center">
							...
							<p className="text-gray-500 dark:text-gray-400">Send your first message to {currentChatId}</p>
						</div>
					</div>
				)}
