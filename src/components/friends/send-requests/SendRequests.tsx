...
export default function SendRequests() {
    ...
    const {data: sentFriendRequests, isLoading, error: error} = useSWR(
        user ? ['sent-friend-requests', FriendRequestStatus.PENDING] : null,
        ([_, args]) =>
            fetcher<FriendRequest[]>(friendsApi.getSentFriendRequests, args)
    )

    useEffect(() => {
        sentFriendRequests && setFriendRequests(sentFriendRequests)
    }, [user]);

    if (error) {
        console.error("Error while getting sent friend requests:", error)
    }
    ...
    return (
        <div
            ...
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Friend requests you've sent that are
                    ...
                    <div className="space-y-4">
                        {friendRequests.map(friendReq => (
                            ...
                            >
                                ...
                                <div
                                    className="px-3 py-1 bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-md text-sm">
                                    Awaiting response
                                </div>
                            </div>
                        ))}
